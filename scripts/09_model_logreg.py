"""09_model_logreg.py — Stage 5: Logistic Regression on embeddings + flags.

Features: all-mpnet-base-v2 768-dim embeddings + 27 regex flags (795 total)
CV: 5-fold StratifiedGroupKFold on train split

Outputs:
  data/interim/oof_train_logreg.parquet   (1,476 rows)
  data/interim/pred_val_logreg.parquet    (503 rows)
  data/models/logreg/model.pkl
  W&B run: tags=[stage:5-zoo, purpose:train, model:logreg, split:oof]
"""
...
log = logging.getLogger("09_model_logreg")
...
def main() -> None:
    ...
    # Scale for LogReg (embeddings already normalized, flags are 0/1)
    ...
    def train_fn(x_mat, y):
        return LogisticRegression(
            max_iter=1000, C=1.0, solver="lbfgs", class_weight=None, random_state=42
        ).fit(x_mat, y)
    ...
    oof_probs, fold_metrics = run_group_cv(x_train_s, y_train, groups, train_fn, predict_fn)
    ...
    final_model = train_fn(x_train_s, y_train)
    train_sec = time.perf_counter() - t0

    t1 = time.perf_counter()
    val_probs = predict_fn(final_model, x_val_s)
    ...
    save_oof(train_df["sentence_id"], oof_probs, "logreg")
    save_val_preds(val_df["sentence_id"], val_probs, "logreg")

    model_dir = MODELS_DIR / "logreg"
    model_dir.mkdir(parents=True, exist_ok=True)
    with open(model_dir / "model.pkl", "wb") as fh:
        pickle.dump({"model": final_model, "scaler": scaler, "feature_cols": feature_cols}, fh)
    log.info("Model saved → %s", model_dir / "model.pkl")
    ...
    run = init_wandb_run(
        ...
        tags=["stage:5-zoo", "purpose:train", "model:logreg", "split:oof"],
        job_type="train",
    )
    wandb.log(
        {
    ...
    artifact = wandb.Artifact(
        "model-logreg",
        type="model",
        description="LogReg on embeddings+flags",
